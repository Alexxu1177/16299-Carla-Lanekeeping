...
class laneEnv(gym.Env):
    ...
    def step(self, action):
        ...
        self._ego_car.apply_control(carla.VehicleControl(throttle = 0.5, steer = self._action))
        ...
        lateral_param = -0.1
        steer_param = -0.2
        # lateral : lane related, steer: steer angle related
        self._reward = 1.0 \
            - 0.1 * abs(observation["lateral_error"][0]) \
            - 0.2 * abs(observation["steer_error"][0])
        # self._reward = lateral_param * abs(observation["lateral_error"][0]) + steer_param * abs(observation["steer_error"][0])
        ...
        if (self._step_count > 20 and speed < 0.1):
            terminated = True
